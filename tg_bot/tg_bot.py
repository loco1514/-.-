...
async def api_get_user(telegram_id: int):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(
                f"Ошибка при получении данных пользователя. Код состояния: {response.status_code}")
            return None, None
    except Exception as e:
        logging.error(str(e))
        return None, None
...
async def api_create_user(telegram_id: int, login: str):
    ...
    try:
        response = requests.post(
            f'{url}?telegramId={telegram_id}&login={login}')
        ...
        if response.status_code == 201:
            ...
            logging.error(
                f"Ошибка при создании пользователя. Код состояния: {response.status_code}")
            return False
    except Exception as e:
        logging.error(str(e))
        return False
...
async def api_get_bookings(telegram_id: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(bookings)
            formatted_bookings = [{'id': booking['id'], 'text': f"{booking['data'][:5]} {booking['startTime']} - {booking['endTime']} floor{booking['meetingRoom']['floor']}"}
                                  for booking in bookings if not booking['canceled']]
            return formatted_bookings
        elif response.status_code == 404:
            logging.error(
                "Бронирования для указанного пользователя не найдены.")
            return []
        else:
            logging.error(
                f"Ошибка при получении бронирований. Код состояния: {response.status_code}")
            return None
    except Exception as e:
        logging.error(str(e))
        return None
...
async def api_cancel_booking(booking_id: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(
                f"Ошибка при отмене бронирования. Код состояния: {response.status_code}")
            return False
    except Exception as e:
        logging.error(str(e))
        return False
...
async def api_get_reports():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(
                f"Ошибка при получении отчетов. Код состояния: {response.status_code}")
            return None
    except Exception as e:
        logging.error(str(e))
        return None
